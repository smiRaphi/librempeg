...
static int baka_probe(const AVProbeData *p)
{
    if (AV_RL32(p->buf) != MKTAG('B','A','K','A') || AV_RL32(p->buf+8) != MKTAG(' ','A','H','O'))
            return 0;

    return AVPROBE_SCORE_MAX;
}

static int baka_read_header(AVFormatContext *s)
{
    ...
    avio_skip(pb, 4);
    if (avio_rl32(pb) != MKTAG(' ','A','H','O') || avio_rl32(pb) != MKTAG('P','A','P','A'))
        return AVERROR_INVALIDDATA;
    ...
    for (;;) {
        ...
        int64_t next_off = avio_tell(pb) + size;

        switch (tag) {
        case MKTAG('P','A','P','A'):
            st->codecpar->block_align = avio_rb16(pb);
            if (st->codecpar->block_align <= 0 || st->codecpar->block_align % 2)
                return AVERROR_INVALIDDATA;
            st->duration = avio_rb32(pb);
            break;
        case MKTAG('M','A','M','A'):
            if (data_offset) {
                av_log(s, AV_LOG_ERROR, "Multiple MAMA (DATA) tags found\n");
                return AVERROR_INVALIDDATA;
            }
    ...
    if (!data_offset) {
        av_log(s, AV_LOG_ERROR, "No MAMA (DATA) tag found\n");
        return AVERROR_INVALIDDATA;
    }
...
static int baka_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    int ret;
    ...
    ret = av_get_packet(s->pb, pkt, FFMIN(block_align, bc->data_end - pos));
    pkt->pos = pos;
...
const FFInputFormat ff_baka_demuxer = {
    .p.name         = "baka",
    .p.long_name    = NULL_IF_CONFIG_SMALL("BAKA (KCET Games)"),
    .read_probe     = baka_probe,
    .read_header    = baka_read_header,
    .read_packet    = baka_read_packet,
};
